...
export function MineProcessFlowchart({ mineId, mineName, activeProcessId }: MineProcessFlowchartProps) {
  ...
  return (
    <div className="w-full bg-[#0c121e]/90 border border-slate-800/80 rounded-2xl p-6 shadow-xl relative overflow-hidden backdrop-blur-md">
      ...
      <div className="overflow-x-auto pb-4 pt-1 scrollbar-thin scrollbar-thumb-slate-700 scrollbar-track-transparent">
        <div className="flex items-stretch gap-3 min-w-[1100px]">
          {MINE_PROCESS_STEPS.map((step, idx) => {
            ...
            let statusBg = 'bg-slate-800/60 text-slate-400 border-slate-700';
            let iconColor = 'text-slate-400';
            let borderHover = 'hover:border-slate-600';
            ...
            return (
              <React.Fragment key={step.id}>
                ...
                <div
                  ...
                  className={`flex-1 min-w-[170px] max-w-[210px] group relative cursor-pointer rounded-xl p-4 transition-all duration-300 border bg-[#090d16]/90 shadow-md ${
                    isSelected 
                      ? 'border-emerald-400 ring-2 ring-emerald-400/20 bg-emerald-950/20 scale-[1.02]' 
                      : `border-slate-800 ${borderHover} hover:scale-[1.02] hover:-translate-y-0.5`
                  }`}
                >
                  ...
                  <div className="flex items-center gap-2.5 mb-2">
                    <div className={`p-2 rounded-lg bg-slate-900 border border-slate-800 ${iconColor} group-hover:scale-110 transition-transform`}>
                      <Icon className="w-4 h-4" />
                    </div>
                    <span className="text-[10px] font-medium text-slate-400 uppercase tracking-wider line-clamp-1">
                      {step.category}
                    </span>
                  </div>

                  {/* Title */}
                  <h3 className="text-xs font-bold text-white mb-2 group-hover:text-emerald-400 transition-colors line-clamp-2 leading-snug">
                    {step.title}
                  </h3>
                  ...
                  <div className="mt-4 pt-2 border-t border-slate-800/60 flex items-center justify-between text-[11px] font-semibold text-slate-400 group-hover:text-emerald-400 transition-colors">
