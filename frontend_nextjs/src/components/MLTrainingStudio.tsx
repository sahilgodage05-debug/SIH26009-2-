...
Map.addLayer(ndvi, {min: 0.1, max: 0.8, palette: ['red', 'yellow', 'green']}, 'NDVI');`;
  ...
  return (
    <div className="flex-1 bg-[#090d16] overflow-y-auto p-4 sm:p-6 space-y-6">
        ...
        <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4">
          <div>
            <div className="flex items-center gap-2 mb-1.5">
              ...
              <span className="text-xs text-slate-400 font-mono">• GEE • NGDR • ISRO Bhuvan</span>
            </div>
          ...
          <div className="flex items-center gap-2 shrink-0">
            <a
              ...
              className="flex items-center gap-2 px-4 py-2.5 rounded-xl bg-gradient-to-r from-emerald-600 to-teal-600 hover:from-emerald-500 hover:to-teal-500 text-white font-bold text-xs shadow-lg shadow-emerald-950/60 transition-all hover:scale-105"
            >
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <div className="p-4 rounded-xl bg-slate-900/70 border border-slate-800">
          <span className="text-xs text-slate-400 block mb-1">Random Forest Accuracy</span>
          <div className="flex items-baseline gap-2">
            <span className="text-3xl font-extrabold text-emerald-400 font-mono">99.8%</span>
            <span className="text-[11px] text-slate-400">on 600 test holdouts</span>
          </div>
          <span className="text-[10px] text-emerald-500 font-medium mt-1 block">ROC-AUC: 0.999 (High Discriminative Power)</span>
        </div>

        <div className="p-4 rounded-xl bg-slate-900/70 border border-slate-800">
          <span className="text-xs text-slate-400 block mb-1">Synthetic Training Points</span>
          <div className="flex items-baseline gap-2">
            <span className="text-3xl font-extrabold text-white font-mono">3,000</span>
            <span className="text-[11px] text-slate-400">GPS Coordinates</span>
          </div>
          <span className="text-[10px] text-slate-400 mt-1 block">705 Positive Deposits • 2,295 Barren Points</span>
        </div>

        <div className="p-4 rounded-xl bg-slate-900/70 border border-slate-800">
          <span className="text-xs text-slate-400 block mb-1">Primary Indicator</span>
          <div className="flex items-baseline gap-2">
            <span className="text-xl font-bold text-teal-300">Bouguer Gravity + NDVI</span>
          </div>
          <span className="text-[10px] text-slate-400 mt-1 block">67.0% Joint Feature Importance</span>
        </div>

        <div className="p-4 rounded-xl bg-slate-900/70 border border-slate-800">
          <span className="text-xs text-slate-400 block mb-1">Earth Observation APIs</span>
          <div className="flex items-baseline gap-2">
            <span className="text-xl font-bold text-amber-400">GEE + GSI NGDR</span>
          </div>
          <span className="text-[10px] text-slate-400 mt-1 block">Sentinel-2, Landsat-9, CHIRPS, Bhuvan</span>
        </div>
      </div>
      ...
      {activeSubTab === 'overview' && (
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
          ...
          <div className="lg:col-span-7 p-5 rounded-2xl bg-slate-900/70 border border-slate-800 shadow-xl space-y-4">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-[11px] text-slate-400">What the model learns to identify Manganese = 1</p>
              </div>
        ...
        <div className="p-5 rounded-2xl bg-slate-900/70 border border-slate-800 shadow-xl space-y-4">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
            ...
            <a
              href="/api/dataset"
              download="moil_manganese_synthetic_training_dataset.csv"
              className="flex items-center gap-2 px-3 py-1.5 rounded-lg bg-emerald-600 hover:bg-emerald-500 text-white font-bold text-xs transition-colors self-start"
            >
              ...
              <tbody className="divide-y divide-slate-800/60 font-sans">
                {previewRows.map(row => (
                  <tr key={row.id} className="hover:bg-slate-800/40 transition-colors">
                    <td className="p-2.5 font-mono text-slate-300 font-bold">{row.id}</td>
                    <td className="p-2.5 font-mono text-slate-400">{row.lat.toFixed(4)}°, {row.lng.toFixed(4)}°</td>
                    <td className="p-2.5 text-slate-200 font-medium">{row.mine}</td>
                    <td className="p-2.5 font-mono text-slate-400">{row.dist}</td>
                    <td className="p-2.5 font-mono">
                      <span className={row.ndvi < 0.35 ? 'text-emerald-400 font-bold' : 'text-slate-400'}>
                        {row.ndvi}
                      </span>
                    </td>
                    <td className="p-2.5 font-mono">
                      <span className={row.gravity.includes('+') ? 'text-amber-400 font-bold' : 'text-slate-400'}>
                        {row.gravity}
                      </span>
                    </td>
                    <td className="p-2.5 font-mono text-slate-300">{row.lst}</td>
      ...
      {activeSubTab === 'predict' && (
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
          ...
          <div className="lg:col-span-5 p-5 rounded-2xl bg-slate-900/70 border border-slate-800 shadow-xl space-y-4">
            <h3 className="text-sm font-bold text-white flex items-center gap-2">
            ...
            <div className="space-y-3">
              <div>
                <label className="text-xs font-semibold text-slate-300 block mb-1">Latitude (°N)</label>
                <input
                  type="number"
                  step="0.0001"
                  value={testLat}
              ...
              <div>
                <label className="text-xs font-semibold text-slate-300 block mb-1">Longitude (°E)</label>
                <input
                  type="number"
                  step="0.0001"
                  value={testLng}
              ...
              {/* Quick Presets */}
              <div className="pt-2">
                <span className="text-[11px] text-slate-400 block mb-1.5">Quick Coordinate Presets:</span>
              ...
              <button
                ...
                className="w-full mt-4 flex items-center justify-center gap-2 py-2.5 px-4 rounded-xl bg-gradient-to-r from-emerald-600 to-teal-600 hover:from-emerald-500 hover:to-teal-500 text-white font-bold text-xs shadow-md shadow-emerald-950/50 transition-all"
              >
          ...
          {inferenceResult && (
            <div className="lg:col-span-7 p-5 rounded-2xl bg-gradient-to-br from-slate-900 via-slate-900/90 to-slate-950 border border-emerald-500/30 shadow-xl space-y-4">
              <div className="flex items-center justify-between">
                <span className="text-xs font-semibold text-slate-400">AI Model Output</span>
              ...
              <div>
                ...
                <span className="text-xs text-slate-400 ml-2">Manganese Deposit Probability</span>
              </div>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 pt-2">
                <div className="p-2.5 rounded-lg bg-slate-950/80 border border-slate-800 text-xs">
                  <span className="text-[10px] text-slate-400 block">Sentinel-2 NDVI</span>
                  <span className="font-mono text-white font-bold">{inferenceResult.ndvi}</span>
                </div>
                <div className="p-2.5 rounded-lg bg-slate-950/80 border border-slate-800 text-xs">
                  <span className="text-[10px] text-slate-400 block">Bouguer Gravity</span>
                  <span className="font-mono text-white font-bold">{inferenceResult.gravity} mGal</span>
                </div>
                <div className="p-2.5 rounded-lg bg-slate-950/80 border border-slate-800 text-xs">
                  <span className="text-[10px] text-slate-400 block">LST Temperature</span>
                  <span className="font-mono text-white font-bold">{inferenceResult.lst}°C</span>
                </div>
                <div className="p-2.5 rounded-lg bg-slate-950/80 border border-slate-800 text-xs">
                  <span className="text-[10px] text-slate-400 block">Soil Moisture</span>
                  <span className="font-mono text-white font-bold">{inferenceResult.soilMoisture}%</span>
                </div>
              </div>
      ...
      {activeSubTab === 'gee-code' && (
        <div className="p-5 rounded-2xl bg-slate-900/70 border border-slate-800 shadow-xl space-y-3">
          ...
          <div className="bg-[#070b12] p-4 rounded-xl border border-slate-800 text-xs font-mono text-emerald-300 overflow-x-auto leading-relaxed">
